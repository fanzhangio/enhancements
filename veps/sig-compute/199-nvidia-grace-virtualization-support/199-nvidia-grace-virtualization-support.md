...
- Explicit dependency on the generic IOMMUFD path defined by [VEP 266](https://github.com/kubevirt/enhancements/pull/267) for host device binding and FD delivery.
...
- **IOMMUFD** ([VEP 266](https://github.com/kubevirt/enhancements/pull/267)) must be available for Grace host-device binding. VEP 199 consumes the generic device plugin, FD delivery, and domain-level `<iommufd>` behavior from VEP 266; it does not redefine those mechanisms.
...
<devices>
  <iommu model='smmuv3'>
    <driver pciBus='...' accel='on' ats='on' ril='off'
            ssidSize='20' oas='48'/>
  </iommu>
</devices>
```
...
When `GraceIOVirtualization` is enabled, the admission webhook applies the following Phase 1 checks to VMIs that include Grace-class GPU host devices:
...
| `dedicatedCpuPlacement` is `true` and `numa.guestMappingPassthrough` is set | Required so the planner can build deterministic guest NUMA cells. |
| All requested Grace host devices resolve to known Grace GPU resources | Prevents Grace handling from silently applying to unrelated devices. |
...
- **Grace-specific IOMMUFD plumbing in this VEP.** Earlier drafts described the IOMMUFD device plugin, FD transport, and per-`<hostdev>` driver semantics. These mechanisms are generic host-device infrastructure and are now owned by [VEP 266](https://github.com/kubevirt/enhancements/pull/267). VEP 199 consumes the VEP 266 IOMMUFD path and limits itself to the Grace-specific guest platform contract built on top of it.
...
- SMMUv3 emission: one `<iommu model='smmuv3'>` per Grace GPU bus, with the Phase 1 attributes (`accel`, `ats`, `ril`, `ssidSize`, `oas`), `cmdqv` not set.
...
- ARM64 Grace Hopper or Grace Blackwell host with SMMUv3 nested translation support and IOMMUFD enabled in the host kernel (Linux >= 6.2 for IOMMUFD, see [VEP 266](https://github.com/kubevirt/enhancements/pull/267)).
